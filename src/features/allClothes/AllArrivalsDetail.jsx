...
} from "@/app/(authHome)/showCartSlice";
...
function AllArrivalsDetail({ data, error }) {
  ...
  function handleAddItemToCart() {
    ...
    if (initialCart.length > 0) {
      const duplicateItems = initialCart.some(
        (el) => el.id === data.id && el.selectedSize === size,
      );
      ...
      if (duplicateItems) {
        toast.error(
          "item is already in cart.. if you want this very item,you can add the quantity or select a different size..",
        );
      } else {
  ...
  if (error)
    toast.error("there was an error loading this item. Refresh and try again");
    ...
    <div className="px-2 py-4 md:px-[2rem] lg:px-[3rem] xl:px-[4rem] 2xl:px-[6rem]">
      ...
      >
        ...
        <div className=" h-fit space-y-2 phones:mt-2 md:h-fit xl:h-[35rem]">
          <h1 className="font-extrabold text-3xl md:text-4xl capitalize">
            {data.name}
          </h1>
          ...
          >
            choose size{" "}
            <span
              className={`md:hidden transition-all duration-100 ${rotate && "rotate-90"}`}
            >
              &nbsp;&gt;
            </span>
          </h2>
          ...
          <motion.div
            layout
            className={`max-h-[fit] grid grid-cols-4 space-y-2 md:space-y-0
          ...
          >
            <motion.p
              className="   transition-colors delay-75 duration-[0.7s] w-fit rounded-xl   bg-[#F0F0F0] hover:bg-black hover:text-white  -75 px-4 py-2  font-medium uppercase"
            ...
            <motion.p
              className="   transition-colors delay-75 duration-[0.7s] w-fit rounded-xl   bg-[#F0F0F0] hover:bg-black hover:text-white   px-4 py-2  font-medium uppercase"
              onClick={handleGetSize}
              data-size="XS"
            >
              xs
          ...
          <p
            className={`${noSizeError === true ? "" : "hidden"} text-red-600 md:pb-2 xl:pb-8`}
          >
          ...
          >
            <div className="flex bg-[#F0F0F0] p-2 rounded-xl space-x-2 w-fit md:w-[30%] border-2 border-solid border-white">
              ...
              <input
                ...
                className="w-[3rem] outline-none  md:w-full bg-[#F0F0F0] text-center placeholder:text-black"
