...
function Filters() {
  ...
  function addRangeToUrl() {
    if (rangeFrom && !rangeTo)
      router.push(pathname + "?" + createQueryString("range", rangeFrom));

    // if(rangeTo && !rangeFrom) router.push(pathname + "?" + createQueryString("to", rangeTo))

    if (rangeFrom && rangeTo) {
      router.push(
        pathname + "?" + createQueryString("range", rangeFrom + "-" + rangeTo),
      );
  ...
  return (
    <div
      className={`  ${showFilter === true ? "translate-x-[0%]" : "translate-x-[-100%]"}
      ...
      <div>
        ...
        <ul>
          <Link
            ...
            href="/casual?page=1"
          >
            <li className="capitalize font-light flex">
              casual &nbsp;
              <LiaExternalLinkAltSolid
                style={{ width: "1rem", height: "1rem", color: "#d6d3d1" }}
              />
            </li>
          </Link>
        ...
        <div className="mt-4">
          <label htmlFor="sort price " className="font-bold">
            sort by price
          </label>
        ...
        <div className="space-y-2 border-2 border-solid bg-stone-50 shadow-lg py-4 mt-4">
          ...
          <div className="flex">
            <label htmlFor="from" className=" font-semibold capitalize">
              {" "}
              from{" "}
            </label>
            <input
              type="number"
              className=" outline outline-1 outline-stone-500 w-[50%]"
              onChange={handleRangeFrom}
            />
          </div>

          <div className="flex">
            <label htmlFor="to" className=" font-semibold capitalize">
              {" "}
              to &nbsp; &nbsp;{" "}
            </label>
            <input
          ...
          {rangeError && !rangeFrom && (
            <p className=" text-red-600 italic">input start price</p>
          )}

          <div
            className={`flex justify-center ${!rangeFrom && "hidden"} transition-all`}
          >
            <button
              className="border-2 border-black font-bold capitalize  w-[80%] rounded-lg m p-2 bg-black text-white md:hover:bg-stone-50 md:hover:text-black md:hover:border-2 md:hover:border-black transition-all"
