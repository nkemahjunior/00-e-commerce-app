...
function LoadMore({numberOfClothes}) {
    ...
    function loadMoreButton(){
      if(curPage === pagenumberOfClothes) return toast.error(" no more clothes to load")
      

        const next = curPage === pagenumberOfClothes ? curPage : curPage + 1

        router.push(pathname + '?' + createQueryString('page',next),{scroll:false})
    ...
    return (
        <div className="flex flex-col space-y-4">
            <div className="flex space-x-4 justify-center ">
                ...
                <button className=" font-bold border-2 border-solid p-2 " onClick={loadMoreButton}>
                    Load more
                </button>

                <button className=" font-bold border-2 border-solid p-2 " onClick={handleScrollToTop}>
                    scroll to top
                </button>
            </div> 
            ...
            <div  className="">
                ...
                <p>
                    showing {1} to {
                        curPage === pagenumberOfClothes ? numberOfClothes : curPage * PAGE_SIZE
